...
def get_args():
    ...
    parser.add_argument('--task', type=str, required=True, 
                        choices=['nlvr2', 'vqav2', 'flickr30k', 'coco_retrieval', 'coco_captioning', 'nocaps', 'imagenet', 'binary_classification', 'cross_entropy', 'language_model_multichoice','language_model_tf'], 
...
def main(args, ds_init):
    ...
    if args.task == "vqav2": # ms
        ...
        if not args.model.endswith(args.task):
            if args.task in ("flickr30k", "coco_retrieval"):
                ...
                model_config = "beit3_large_patch16_480_captioning"
            else:
                model_config = "%s_%s" % (args.model, args.task)
        else:
            model_config = args.model
    ...
    if args.eval:
        ...
        if args.task in ["nlvr2", "flickr30k", "coco_retrieval", "imagenet"]:
            ...
            predictions, _ = evaluate(data_loader_test, model, device, task_handler)
            exit(0)
        elif args.task == "cross_entropy":
            predictions, _ = evaluate(data_loader_test, model, device, task_handler)
            # save csv file
            csv_file = os.path.join(args.output_dir, "cross_entropy_predictions.csv")
            if utils.is_main_process():
                print("Cross Entropy Predictions:")
                print(predictions)
                
                import pandas as pd
                
                ce = pd.DataFrame(predictions).to_csv(csv_file, index=False)
                ce.to_csv(csv_file, index=False)
            exit(0)
