...
from datasets import create_downstream_dataset, create_downstream_dataset_only_train
...
def get_args():
    ...
    parser.add_argument('--task', type=str, required=True, 
                        choices=['nlvr2', 'vqav2', 'flickr30k', 'coco_retrieval', 'coco_captioning', 'nocaps', 'imagenet', 'binary_classification', 'cross_entropy', 'language_model_tf', 'language_model_multichoice', 'language_model_multichoice_prefix'], 
...
def main(args, ds_init):
    ...
    print(f"Number of training samples: {len(data_loader_train.dataset)}")
    print(f"Number of validation samples: {len(data_loader_val.dataset)}")
    ...
    if args.task == "vqav2": # ms
        model_config = args.model
    else:
        # 기존 로직은 다른 태스크를 위해 그대로 둡니다.
        if not args.model.endswith(args.task):
            if args.task in ("flickr30k", "coco_retrieval"):
                model_config = "%s_retrieval" % args.model
            elif args.task in ("coco_captioning", "nocaps"):
                ...
                model_config = "%s_imageclassification" % args.model
            elif args.task in ("binary_classification", "cross_entropy"):
                model_config = args.model
            else:
                model_config = "%s_%s" % (args.model, args.task)
        else:
    ...
    # model inference 파라미터 출력
    if args.task in ["nlvr2", "flickr30k", "coco_retrieval", "imagenet"]:
        print("Inference parameters: input_size = {}, task = {}, batch_size = {}".format(
            args.input_size, args.task, args.batch_size))
    elif args.task in ["vqav2", "coco_captioning", "nocaps"]:
        print("Inference parameters: input_size = {}, task = {}, batch_size = {}".format(
            args.input_size, args.task, args.eval_batch_size))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #     data_loader_train = create_downstream_dataset_only_train(args, epoch)
        ...
        if data_loader_val is not None:
            ...
            elif args.task in ["language_model_multichoice", "language_model_tf", 'language_model_multichoice_prefix']:
                test_stats, task_key = evaluate(data_loader_val, model, device, task_handler, args.task)
                if log_writer is not None:
    ...
    # test
    data_loader_test = create_downstream_dataset(args, is_eval=True)
    print(f"Start testing for {args.task} task")
    if args.task in ["binary_classification", "cross_entropy"]:
        test_stats, task_key = evaluate(data_loader_test, model, device, task_handler)

        print(f"Accuracy of the network on the {len(data_loader_test.dataset)} test images: {test_stats[task_key]:.3f}%")
        # save csv ID, label, prediction
        # if args.output_dir and utils.is_main_process():
            # utils.save_binary_classification_predictions(
            #     args.output_dir, data_loader_test.dataset, test_stats['predictions'], task_key)
