...
class TransformerModel(TransformerModelBase):
    ...
    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens, encoder_layers=None):
        return super().build_decoder(
            TransformerConfig.from_namespace(args), tgt_dict, embed_tokens, encoder_layers=encoder_layers
        )
...
@register_model_architecture("transformer", "transformer_edge")
def edge_architecture(args):
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    ...
    args.model_param_type = getattr(
        args, "model_param_type", "edgeformer"
    )
