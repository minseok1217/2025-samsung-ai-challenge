...
# --- START OF NEW CODE FOR 4-CHOICE VQA ---
class BEiT3ForVisualQuestionAnsweringFourChoice(BEiT3Wrapper):
    """
    BEiT3 model for Visual Question Answering with a 4-choice classification head.
    This class is specifically designed to handle VQA tasks where the answer is one of four choices (A, B, C, D).
    """
    def __init__(
            self, 
            args, 
            num_choices=4, # 4지선다형으로 고정하거나 기본값으로 설정합니다.
            norm_layer=nn.LayerNorm, 
            **kwargs
    ):
        super(BEiT3ForVisualQuestionAnsweringFourChoice, self).__init__(args=args)
        embed_dim = args.encoder_embed_dim
        self.pooler = Pooler(
            input_features=embed_dim, 
            output_features=embed_dim, 
        ...
        self.head = nn.Sequential(
            ...
            norm_layer(embed_dim * 2), 
            nn.GELU(), 
            nn.Linear(embed_dim * 2, num_choices), # 최종 출력 로짓 수를 4로 설정합니다.
        )
        self.head.apply(self._init_weights)

    def forward(self, image, question, padding_mask, **kwargs):
        # 이 forward 메서드는 BEiT3ForVisualQuestionAnswering와 동일하게 동작합니다.
        # 차이점은 __init__에서 head의 출력 크기가 4로 설정된다는 점입니다.
        outputs = self.beit3(
            textual_tokens=question, 
            visual_tokens=image, 
            text_padding_position=padding_mask, 
        ...
        x = outputs["encoder_out"]
        cls_rep = self.pooler(x)
        return self.head(cls_rep) # [batch_size, 4] 형태의 로짓을 출력합니다.
...
# --- START OF NEW MODEL REGISTRATIONS FOR 4-CHOICE VQA ---
@register_model
def beit3_base_patch16_384_vqav2_4choice(pretrained=False, **kwargs):
    args = _get_base_config(img_size=384, **kwargs)
    args.normalize_output = False
    model = BEiT3ForVisualQuestionAnsweringFourChoice(args, num_choices=4, **kwargs)
    return model


@register_model
def beit3_base_patch16_480_vqav2_4choice(pretrained=False, **kwargs):
    args = _get_base_config(img_size=480, **kwargs)
    args.normalize_output = False
    model = BEiT3ForVisualQuestionAnsweringFourChoice(args, num_choices=4, **kwargs)
    return model
...
@register_model
def beit3_large_patch16_384_vqav2_4choice(pretrained=False, **kwargs):
    args = _get_large_config(img_size=384, **kwargs)
    args.normalize_output = False
    model = BEiT3ForVisualQuestionAnsweringFourChoice(args, num_choices=4, **kwargs)
    return model
...
@register_model
def beit3_large_patch16_480_vqav2_4choice(pretrained=False, **kwargs):
    args = _get_large_config(img_size=480, **kwargs)
    args.normalize_output = False
    model = BEiT3ForVisualQuestionAnsweringFourChoice(args, num_choices=4, **kwargs)
    return model
...
@register_model
def beit3_large_patch16_768_vqav2_4choice(pretrained=False, **kwargs):
    args = _get_large_config(img_size=768, **kwargs)
    args.normalize_output = False
    model = BEiT3ForVisualQuestionAnsweringFourChoice(args, num_choices=4, **kwargs)
    return model
# --- END OF NEW MODEL REGISTRATIONS ---
