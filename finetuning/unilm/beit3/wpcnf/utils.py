...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        if not iterable or len(iterable) == 0: # ms
            return
    ...
    if loss_scaler is not None:
        ...
        if args.resume:
            if args.resume.startswith('https'):
                ...
                # checkpoint = torch.load(args.resume, map_location='cpu') # ms
...
def load_model_and_may_interpolate(ckpt_path, model, model_key, model_prefix):
    # --- PyTorch UnpicklingError 해결을 위한 추가 코드 시작 ---
    # PyTorch 2.6+에서 'weights_only=True' 기본값으로 인해 발생하는 문제를 해결하기 위해
    # numpy.core.multiarray.scalar를 안전한 전역 객체로 추가합니다.
    # 이 부분은 torch.load를 호출하기 전에 한 번만 실행되면 됩니다.
    torch.serialization.add_safe_globals([np.core.multiarray.scalar])
    # --- PyTorch UnpicklingError 해결을 위한 추가 코드 끝 ---

    if ckpt_path.startswith('https'):
        checkpoint = torch.hub.load_state_dict_from_url(
            ckpt_path, map_location='cpu', check_hash=True)
    else:
        # 기존 torch.load 호출. 이제 add_safe_globals 덕분에 UnpicklingError가 발생하지 않을 것입니다.
        checkpoint = torch.load(ckpt_path, map_location='cpu', weights_only=False)

    print("Load ckpt from %s" % ckpt_path)
    checkpoint_model = None
    for model_key_item in model_key.split('|'): # 변수명 충돌 방지를 위해 model_key_item으로 변경
        if model_key_item in checkpoint:
            checkpoint_model = checkpoint[model_key_item]
            print("Load state_dict by model_key = %s" % model_key_item)
            break
    ...
    state_dict = model.state_dict()
    for k in ['head.weight', 'head.bias']:
        # 이 부분은 모델의 head가 pretrained 모델의 head와 모양이 다를 때 제거하는 로직입니다.
        # VMCBench 파인튜닝 시 VQA head의 크기가 4로 변경되었으므로, 이 로직은 유지되어야 합니다.
        if k in checkpoint_model and checkpoint_model[k].shape != state_dict[k].shape:
    ...
    for pos_embed_key in ("vision_pos_embed", "pos_embed", "beit3.encoder.embed_positions.A.weight"):
        if pos_embed_key in checkpoint_model:
            ...
            if pos_embed_key == "beit3.encoder.embed_positions.A.weight":
                # being consistent with Fairseq, which starts from 2 for position embedding
                torchscale_model = True
                # 여기서 model.beit3.vision_embed가 없다면, 다른 모델 구조를 참조해야 합니다.
                # BEiT-3 Large 모델 구조에 따라 다를 수 있습니다.
                num_patches = model.beit3.vision_embed.num_patches 
                num_extra_tokens = model.beit3.vision_embed.num_position_embeddings() + 2 - num_patches
            else:
                torchscale_model = False
                # 이 부분은 BEiT-3 Large가 아닐 경우의 처리이므로, 필요시 확인
                # model.patch_embed가 없을 수 있습니다.
                # getattr(model, 'patch_embed', None) 등으로 안전하게 접근하거나,
                # BEiT-3 Large에 맞는 정확한 위치 임베딩 패스를 확인해야 합니다.
                # 여기서는 원본 코드 유지
                num_patches = model.patch_embed.num_patches 
                num_extra_tokens = getattr(model, pos_embed_key).shape[-2] - num_patches
    ...
    # load_state_dict 함수가 정의되어 있어야 합니다 (utils.py 또는 기타).
    load_state_dict(model, checkpoint_model, prefix=model_prefix)
