...
class VQAv2Dataset(BaseDataset):
    ...
    def make_dataset_index(cls, data_path, tokenizer, annotation_data_path):
        ...
        # 모든 답변 포함하도록 수정
        all_major_answers = [normalize_word(q["multiple_choice_answer"]) for q in annotations_train2014 + annotations_val2014]
        counter = {k: v for k, v in Counter(all_major_answers).items()}
        ans2label = {k: i for i, k in enumerate(counter.keys())}
            ...
            for q in annots:
                ...
                for answer in answer_count:
                    if answer not in ans2label:
                        continue
                    labels.append(ans2label[answer])
                    scores.append(cls.get_score(answer_count[answer]))

                if q["image_id"] in _annot and q["question_id"] in _annot[q["image_id"]]:
                    _annot[q["image_id"]][q["question_id"]]["labels"] = labels
                    _annot[q["image_id"]][q["question_id"]]["scores"] = scores
        ...
        for split in ["train", "val", "test", "test-dev"]:
            ...
            print("all images have caption annotations" if len(paths) == len(annot_paths) else "not all images have caption annotations")
            print(len(paths), len(annot_paths), len(annot))
            ...
            split2items[split] = items
            _write_data_into_jsonl(items, os.path.join(data_path, f"vqa.{split}.jsonl"))
        ...
        _write_data_into_jsonl(trainable_val, os.path.join(data_path, "vqa.trainable_val.jsonl"))
        _write_data_into_jsonl(rest_val, os.path.join(data_path, "vqa.rest_val.jsonl"))

        with open(os.path.join(data_path, "answer2label.txt"), "w", encoding="utf-8") as writer:
            for ans in ans2label:
                writer.write(json.dumps({"answer": ans, "label": ans2label[ans]}) + "\n")
