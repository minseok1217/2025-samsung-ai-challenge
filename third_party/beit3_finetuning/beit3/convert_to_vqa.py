...
def create_vqa_format_strict_vqav2(
    csv_path,
    image_root,
    output_img_dir,
    vqa_dir,
    ...
    print(f"🔄 변환 중: {split} → VQA format (strict)")
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if "label" in row and pd.notna(row["label"]):
            ...
            assert label in ["A", "B", "C", "D"], f"Unexpected label: {label}"
            annotations.append({
                "question_id": question_id,
                "image_id": image_id,
                "answer_type": "other",
    ...
    # 질문 JSON 저장
    questions_json = {
        "info": {"description": f"{split} classification-to-vqa (strict)"},
    ...
    print(f"✅ {split} 변환 완료 → 질문: {questions_fname}")


if __name__ == "__main__":
    create_vqa_format_strict_vqav2(
        csv_path="data/train.csv",
        image_root="data/train_input_images",
        output_img_dir="data_vqaformat_5/train2014",
        vqa_dir="data_vqaformat_5/vqa",
        split="train",
        coco_prefix="COCO_train2014",
        start_image_id=0,
    )
    ...
    create_vqa_format_strict_vqav2(
        ...
        output_img_dir="data_vqaformat_5/val2014",
        vqa_dir="data_vqaformat_5/vqa",
        split="val",
    ...
    create_vqa_format_strict_vqav2(
        ...
        output_img_dir="data_vqaformat_5/test2015",
        vqa_dir="data_vqaformat_5/vqa",
        split="test",
