...
def load_model_and_may_interpolate(
    ...
    model_prefix="",
    logger=None, # 로깅을 위해 logger 인자 추가 (선택 사항)
):
    if logger is None:
        import logging
    ...
    if pretrained_ckpt is not None:
        logger.info("Load ckpt from %s" % pretrained_ckpt)
        checkpoint = torch.load(pretrained_ckpt, map_location='cpu')

        if model_key is not None and model_key in checkpoint:
            logger.info("Load state_dict by model_key = %s" % model_key)
            state_dict = checkpoint[model_key]
        elif model_key is not None and model_key.replace('|', '.') in checkpoint:
            logger.info("Load state_dict by model_key = %s" % model_key.replace('|', '.'))
            state_dict = checkpoint[model_key.replace('|', '.')]
        else:
            state_dict = checkpoint
        
        # strip prefix of state_dict
        if model_prefix:
            logger.info(f"Strip model_prefix = {model_prefix}")
            state_dict = {k.replace(model_prefix, ""): v for k, v in state_dict.items() if k.startswith(model_prefix)}
        ...
        if 'pos_embed' in state_dict:
            ...
            # orig_size = int((pos_embed_checkpoint.shape[-2] - num_extra_tokens) ** 0.5)
            # new_size = int(num_patches ** 0.5)
            # Patch size 계산 (모델 설정에서 가져와야 함, 예를 들어 patch_size=16)
            # 현재 input_size 480 / patch_size 16 = 30x30 => 900
            # 기존 모델 224 / 16 = 14x14 => 196
            orig_size_tokens = pos_embed_checkpoint[:, num_extra_tokens:].shape[1] # [CLS] 등 제외한 토큰 수
            orig_h = orig_w = int(orig_size_tokens ** 0.5) # 원본 이미지 패치 그리드 크기
            
            new_h = new_w = int(num_patches ** 0.5) # 새 이미지 패치 그리드 크기 (480 / 16 = 30)

            logger.info(f"Position interpolate from {orig_h}x{orig_w} to {new_h}x{new_w}")
            
            # --- START OF FIX ---
            cls_token = pos_embed_checkpoint[:, :num_extra_tokens]
            
            # 보간 연산 전에 pos_tokens를 float32로 변환
            # (orig_h, orig_w) -> (new_h, new_w)
            # VRAM이 충분한지 확인하세요.
            pos_tokens = pos_embed_checkpoint[:, num_extra_tokens:].reshape(-1, orig_h, orig_w, embedding_size).permute(0, 3, 1, 2)
            
            # 원래 데이터 타입 저장
            original_dtype = pos_tokens.dtype
            
            # interpolate 연산은 FP16에서 bicubic 지원 안함 -> float32로 변환
            ...
            # 연산 후 원래 데이터 타입으로 복원 (bfloat16 또는 float16)
            pos_tokens = pos_tokens.to(original_dtype)
            
            pos_tokens = pos_tokens.permute(0, 2, 3, 1).flatten(1, 2)
            updated_pos_embed = torch.cat((cls_token, pos_tokens), dim=1)
            state_dict['pos_embed'] = updated_pos_embed
            # --- END OF FIX ---
...
def merge_batch_tensors_by_dict_key(batch):
    """
    Collate function to merge batch samples into a single dictionary of tensors.
    Handles nested dictionaries for 'net_input' and non-tensor types.
    """
    if not batch:
        return {}

    # 첫 번째 샘플을 기준으로 키들을 가져옵니다.
    first_sample = batch[0]
    
    # 배치된 텐서를 저장할 딕셔너리 초기화
    batch_tensors = {}
    
    # net_input은 특별히 처리 (중첩된 딕셔너리)
    if 'net_input' in first_sample and isinstance(first_sample['net_input'], dict):
        batch_tensors['net_input'] = {}
        for net_input_key in first_sample['net_input'].keys():
            # net_input 내의 각 항목도 리스트로 모아서 배치 처리
            values_to_collate = [d['net_input'][net_input_key] for d in batch if 'net_input' in d and net_input_key in d['net_input']]
            
            # None이 아닌 값이 있고, Tensor로 변환 가능한 경우에만 변환
            if values_to_collate and isinstance(values_to_collate[0], torch.Tensor):
                batch_tensors['net_input'][net_input_key] = torch.stack(values_to_collate)
            # 다른 유형 (예: Python 리스트의 리스트, 텐서가 아닌 다른 것)은 스킵하거나 별도 처리 필요
            # 현재 이미지와 텍스트 토큰은 이미 Tensor로 반환되므로 torch.stack 사용.
            # padding_mask도 Tensor이므로 stack 사용.
            else:
                # 이 경우 values_to_collate가 Tensor가 아닌 다른 타입일 수 있으므로 주의.
                # 예를 들어, 이미지 경로 문자열 리스트는 여기에 해당할 수 있습니다.
                # VQAv2FourChoiceDataset에서 'image'는 이미 PIL Image -> Tensor 변환되므로 이 else에 들어오지 않음.
                batch_tensors['net_input'][net_input_key] = values_to_collate # 텐서가 아니면 그냥 리스트로 남겨둡니다.

    # 최상위 레벨의 다른 키들을 처리
    for tensor_key in first_sample.keys():
        if tensor_key == 'net_input':
            continue # 이미 처리했으므로 건너_get_text_segment기
        
        # 'id', 'target', 'question_id', 'original_correct_answer', 'choices'
        # id, target, question_id는 torch.tensor로 변환 가능
        # original_correct_answer (str), choices (list)는 직접 변환 불가
        
        values_to_collate = [d[tensor_key] for d in batch]
        
        # Tensor로 변환 가능한 타입인지 확인
        if values_to_collate and (isinstance(values_to_collate[0], (int, float, bool, torch.Tensor)) or \
                                 (isinstance(values_to_collate[0], list) and \
                                  all(isinstance(x, (int, float, bool)) for x in values_to_collate[0]))):
            
            # target이나 question_id 같은 단일 정수/float는 stack 대신 tensor로 직접 생성
            # 이미 텐서인 경우는 그냥 사용
            if isinstance(values_to_collate[0], torch.Tensor):
                batch_tensors[tensor_key] = torch.stack(values_to_collate)
            else: # int, float 등 스칼라 값의 리스트
                # VQAv2FourChoiceDataset에서 target은 이미 torch.tensor로 반환되므로 여기로 올 일 없음.
                # qid (question_id)는 int 이므로 여기로 와서 tensor로 변환됩니다.
                batch_tensors[tensor_key] = torch.tensor(values_to_collate, dtype=torch.long)
        else:
            # Tensor로 변환할 수 없는 타입 (문자열, 복잡한 리스트/딕셔너리 등)은 그대로 유지
            # 예를 들어, 'original_correct_answer' (str), 'choices' (list of str)
            batch_tensors[tensor_key] = values_to_collate
